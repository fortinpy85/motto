...
# TEMPORARY: Authentication disabled for local testing (remove for production!)
def app_access_required(app_handle):
    def decorator(func):
        @wraps(func)
        def wrapper(request, *args, **kwargs):
            # TEMPORARY: Skip authentication check for local testing
            # if not request.user.is_authenticated:
            #     logger.info("User is not authenticated", category="security")
            #     return robust_redirect(request, reverse("index"))

            # TEMPORARY: Skip permission check for local testing
...
def permission_required(
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            # TEMPORARY: Skip permission check for local testing - always allow access
            return view_func(request, *args, **kwargs)

            # ORIGINAL CODE BELOW (commented out for local testing)
